...
- NO Fisher-z transform. We average split r's in raw r-space (nanmean).
...
def compute_split_half_no_fisher(
    series_by_session,
    ses_list,
    ...
    """
    No Fisher-z:
      - compute r for each split on common events
      - apply Spearman–Brown to each split r (optional)
      - average split r's (and/or split SB r's) in raw space
      - compute final noise ceiling = sqrt(max(avg_SB, 0))  (or sqrt(max(avg_r,0)) if SB off)

    Returns dict with:
      r_splits, r_sb_splits,
      r_mean, r_sb_mean,
      noise_ceiling (sqrt of avg reliability, clipped at 0),
      split_ns, split_debug
    """
    ...
    # Mean in raw r space (no Fisher-z)
    good_r = [r for r in r_splits if np.isfinite(r)]
    r_mean = float(np.mean(good_r)) if good_r else np.nan

    good_rsb = [r for r in r_sb_splits if np.isfinite(r)]
    r_sb_mean = float(np.mean(good_rsb)) if good_rsb else np.nan

    # Final noise ceiling AFTER averaging (what you requested)
    # Use SB mean if available, otherwise use raw mean.
    reliability_for_ceiling = r_sb_mean if np.isfinite(r_sb_mean) else r_mean
    reliability_for_ceiling = max(float(reliability_for_ceiling), 0.0) if np.isfinite(reliability_for_ceiling) else np.nan
    noise_ceiling = float(np.sqrt(reliability_for_ceiling)) if np.isfinite(reliability_for_ceiling) else np.nan
...
def parse_args():
    ap = argparse.ArgumentParser(
        "Split-half reliability (4 sessions) for LSS betas within a Glasser ROI, aligned by tc (NO Fisher-z)."
    )
...
def main():
    ...
    for taskdir in task_dirs:
        ...
        enc_stats = compute_split_half_no_fisher(
            series_by_session_enc, ses_list,
            apply_spearman_brown=args.spearman_brown,
            verbose=args.verbose
        )
        del_stats = compute_split_half_no_fisher(
    ...
    out_tsv = os.path.join(args.out_dir, "split_half_roi_reliability_sqrt.tsv")
