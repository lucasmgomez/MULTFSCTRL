...
# Where logs go
LOG_DIR="${NEURAL_BASE}/glm_runs/lsa/64kDense/${SUB}/_logs"
mkdir -p "${LOG_DIR}"

# tmux session name
TMUX_SESSION="lsa_${SUB}"
...
TASKS=(
    ...
    "2back|ctg|ctg|01|2"
    # InterDMS2
    "interdms2|loc_ABBCCA|locABBCCA|01|2"
    "interdms2|ctg_ABBCCA|ctgABBCCA|01|2"
    "interdms2|loc_ABCABC|locABCABC|01|2"
    # ctxdm
    "ctxdm|lol|lol|01|2"
    "ctxdm|olc|olc|01|2"
    # dms
...
spawn_job () {
  ...
  local DT_FILE
  local CONF_TSV
  local LSA_EVENTS_TSV
  local OUT_DIR

  # dtseries/confounds filenames
  ...
  # LSA events path (ONE TSV per run)
...
echo "LSA_EVENTS_TSV=${LSA_EVENTS_TSV}"
...
[[ -f "${LSA_EVENTS_TSV}" ]] || { echo "ERROR missing LSA events TSV: ${LSA_EVENTS_TSV}"; exit 3; }
[[ -f "${CONF_TSV}" ]] || { echo "ERROR missing confounds: ${CONF_TSV}"; exit 4; }

${PIN_PREFIX}"\${PY}" "${LSA_SCRIPT}" \
  --dtseries "${DT_FILE}" \
  --lsa_events_tsv "${LSA_EVENTS_TSV}" \
