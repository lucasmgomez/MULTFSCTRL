...
def select_data(betas, acts, phase2predict='encoding', save_per_run=False, cache_dir=None):
    ...
    # Ensure cache dir exists if we are saving
    ...
    for run_name in betas.keys():
        ...
        # Iterate over TCs in this run
        # Note: We iterate sorted keys to keep deterministic order for per-run files
        ...
        # Concatenate this run's data
        run_betas_stack = np.concatenate(run_betas_list, axis=0)
        run_acts_stack = np.concatenate(run_acts_list, axis=1) # Acts are (Layers, Tokens, Units) usually stacked on dim 1 (Tokens)
